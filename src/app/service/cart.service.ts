...
import{ cart} from '../module/cart/cart.module';
...
addToCart(product: ProductModule,quantity:number){
  const productNumber= parseInt('quantity')
  const x = this.items.products.findIndex((p)=>p.id=== product.id)
  const totalPayment =product.price*productNumber
  if (x >=0){
    this.items.products[x].totalPrice +=totalPayment
    this.items.products[x].quantity += productNumber
  }else{
    let productInCart={
      id:product.id,
      name :product.name,
